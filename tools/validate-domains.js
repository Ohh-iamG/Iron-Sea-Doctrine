...
files.forEach(file => {
  ...
  const hasLevel2 = fileContent.includes("BIOPHYSICAL OS") || fileContent.includes("LIFE OS");
  if (!hasLevel2) {
    console.warn(`[METRIC ADVISORY] Document '${file}' lacks explicit reference to Level 2 Custodianship (BIOPHYSICAL/LIFE OS)`);
  }

  const otherFrameworks = ["INDUSTRIAL OS", "ATMOSPHERIC OS", "CONSTELLATION OS", "CORE OS"];
  otherFrameworks.forEach(os => {
    if (!fileContent.includes(os)) {
